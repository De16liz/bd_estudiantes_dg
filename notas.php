...
class Nota
{
    ...
    private static function obtenerCalificacion($notas, $id_competencias)
    {
        foreach ($notas as $nota) {
            if (intval($nota["id_competencias"]) === intval($id_competencias)) {
                return $nota["notas"];
            }
        }
        return 0;
    ...
    {
        global $mysqli;
        $sentencia = $mysqli->prepare("DELETE FROM notas_aprendices_notas WHERE id_aprendices = ? AND id_competencias = ?");
